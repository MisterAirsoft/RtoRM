...
public class Vue {
	...
	public void extractAndDisplayAlbumArt(String folderPath, int columns, int imageSize) throws IOException {
		...
		// Créer un nouveau JPanel pour les boutons et utiliser GridBagLayout pour
		// qu'ils prennent toute la hauteur de la page
		JPanel buttonPanel = new JPanel(new GridBagLayout());
		buttonPanel.setBackground(Color.BLACK); // Définir la couleur de fond du panneau des boutons

		// Créer un objet GridBagConstraints pour configurer le positionnement et le
		// dimensionnement des composants dans GridBagLayout
		...
		favoritesButton
				.setPreferredSize(new Dimension(preferredButtonWidth, favoritesButton.getPreferredSize().height));
	...
	void afficherMusiques(JPanel panel, TreeMap<String, Musique> musiques, int imageSize, String searchTerm) {
		...
		for (Map.Entry<String, Musique> entry : musiques.entrySet()) {
			...
			if (musique.titre.toLowerCase().contains(searchTerm.toLowerCase())
				...
				titleLabel.setBorder(BorderFactory.createEmptyBorder(borderSize * 10, borderSize, 0, borderSize)); // Bordure
																													// à
																													// droite
																													// du
																													// titre
				artistLabel.setBorder(BorderFactory.createEmptyBorder(borderSize * 10, borderSize, 0, borderSize)); // Bordure
																													// à
																													// gauche
																													// et
																													// droite
																													// de
																													// l'artiste
				albumLabel.setBorder(BorderFactory.createEmptyBorder(borderSize * 10, borderSize, 0, 0)); // Bordure à
																											// gauche de
																											// l'album
	...
	void afficherMusiques(JPanel panel, TreeMap<String, Musique> musiques, int imageSize, String searchTerm,
		...
		for (Map.Entry<String, Musique> entry : musiques.entrySet()) {
			...
			if ((filter.equals("all") && (musique.titre.toLowerCase().contains(searchTerm.toLowerCase())
					|| musique.artist.toLowerCase().contains(searchTerm.toLowerCase())
					|| musique.album.toLowerCase().contains(searchTerm.toLowerCase())))
			...
			if (matchFilter) {
				...
				favButton.setSize(100, 100); // Définir une taille plus petite si nécessaire
		...
		if (!resultFound) {
			...
			noResultLabel.setFont(new Font("Helvetica Neue Light", Font.BOLD, 50)); // Définir une taille de police plus
																					// grande
